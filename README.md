...
> Note: If Prisma Client is not generated yet in your environment, the app falls back to an in-memory store so UI/API smoke testing can still run. For real persistence, run `npx prisma generate` and `npx prisma migrate dev`.
